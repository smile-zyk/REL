...
| 14 | `?:` | 条件运算符（三元运算符，右结合） | `expr ? expr : expr` |
> 说明：`expr_list` 为用 `,`（逗号）分割的表达式序列（也可能是单个 `expr`）。逗号仅用于列表分隔，不作为独立运算符。

> 说明：`if (...) then ... else ...` 是一个 primary，由关键字定界，可像普通操作数一样直接出现在任意运算符上下文中，无需括号，例如 `a + if (c) then 1 else 2`。三元 `?:` 是优先级最低的运算符（与 C 一致），在更高优先级的上下文中嵌入时需加括号，例如 `a + (c ? 1 : 2)`。
...
- `::` 仅可用于矩阵索引、sweep 索引、sweep/matrix 生成器。
- 可用于 `[]` 索引与 `()` 索引，例如 `a[1::1::3,3]`、`a(1::1::3,3)`。
- `a(1::1::3,3)` 中的 `a` 按矩阵对象解释为索引目标，不按函数名解释。
- 可直接用于 `[]` 和 `{}` 生成 sweep 或矩阵，例如 `[1::1::3]`、`{1::1::3}`。
- 在索引上下文中，裸 `::` 表示“该sweep维度全选”，例如 `a[::,1]`。
- 在生成器上下文中，裸 `::` 表示空序列，因此 `[::]` 生成空 sweep，`{::}` 生成空矩阵。
